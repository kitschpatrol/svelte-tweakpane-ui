...
It's open-source, MIT-licensed, extensively documented, and is compatible with Svelte 4 and Svelte 5. It should work anywhere Svelte works, including [SvelteKit](https://svelte.dev/) and "island" frameworks like [Astro](https://astro.build/). _Svelte Tweakpane UI_ is distributed as an [NPM package](https://www.npmjs.com/package/svelte-tweakpane-ui), and the source code is available on [GitHub](https://github.com/kitschpatrol/svelte-tweakpane-ui).
...
_Svelte Tweakpane UI_ takes a monolithic approach. A single dependency and import delivers access to all components, plus controls from several Tweakpane Plugins, and additional Svelte-specific functionality. Tree shaking should keep your bundle size in check (but note the [requisite import style](/svelte-tweakpane-ui/docs#importing-components)).
...
Vanilla Tweakpane conveniently provides two-way bindings for manipulating object properties, but _Svelte Tweakpane UI_ leaves binding up to the user via [Svelte's conventional approach](https://svelte.dev/docs/svelte/bind#bind:property-for-components). This makes bindings explicit, and allows most components to function either as monitors or inputs depending on whether their value property is bound.
...
- [svelte-pane](https://github.com/matiadev/svelte-pane) by [Matija](https://matia.xyz/)
...
- [solid-tweakpane](https://github.com/0x241F31/solid-tweakpane)
...
Note that the _Svelte Tweakpane UI_ must continue to use Svelte 4 internally to package the library components since there does not appear to be a way to generate Svelte 4-compatible components with Svelte 5. Even though the library uses Svelte 4's "legacy" syntax exclusively, Svelte 5 packages components as [functions instead of classes](https://svelte.dev/docs/svelte/v5-migration-guide#Components-are-no-longer-classes), breaking backwards compatibility.
