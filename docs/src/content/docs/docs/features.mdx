...
Note that complete functionality of this feature in the `<CubicBezier>` component is pending the merge of a [pull request](https://github.com/tweakpane/plugin-essentials/pull/19). You may see some glitches with scaling graphs and the `<CubicBezier>` component until then.
