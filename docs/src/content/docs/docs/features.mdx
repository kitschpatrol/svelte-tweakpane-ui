...
uses the Popover API's top layer, so it won't be clipped by the pane. The label—or the whole blade
when unlabeled—also receives the same text as a native `title` tooltip:
