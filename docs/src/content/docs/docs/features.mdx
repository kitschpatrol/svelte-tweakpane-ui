...
With a single prop, you can make a `<Pane>` draggable and resizable. Position and size are persisted in local storage. (This is the default behavior for new `<Pane>`s.)
...
This is particularly useful for interoperability with "tuple-forward" libraries like [Three.js](https://threejs.org) / [Threlte](https://threlte.xyz).
...
Note that complete functionality of this feature is contingent on [a PR](https://github.com/cocopon/tweakpane/pull/586) or [two](https://github.com/pulls) being merged. You may see some glitches with scaling graphs and the `<CubicBezier>` component until then.
...
Svelte Tweakpane UI full manages the lifecycle of the underlying Tweakpane objects, obviating the need for manual calls to [dispose](https://tweakpane.github.io/docs/misc/#disposing).
