...
Controls accept a `description` prop for additional context without adding persistent visual clutter.
The description appears in a compact, theme-aware hint after a short delay when the control's label
is hovered, positioned beside the pointer. When the control receives keyboard focus, the hint is
anchored to the control instead. For an unlabeled control, the whole blade is the hover target. It
uses the active theme's built-in slider value tooltip styling and the Popover API's top layer, so it
won't be clipped by the pane. A theme-matched caret points back to the originating label or focused
control. The label—or the whole blade when unlabeled—also receives the same text as a native
`title` tooltip:
...
The default hover delay is 500ms. Hints appear without a fade and fade out over 500ms. Appearance
defaults inherit from the active Tweakpane theme, but every value can be overridden independently
with an STUI theme key:
...
<script lang="ts">
  ...
  const theme: Theme = {
    ...
    stuiDescriptionBackgroundColor: 'navy',
    stuiDescriptionForegroundColor: 'white',
...
The corresponding inherited CSS custom properties can also be set directly on a pane or any of
its ancestors:

| Theme key                        | CSS custom property                    | Default                          |
| -------------------------------- | -------------------------------------- | -------------------------------- |
| `stuiDescriptionBackgroundColor` | `--stui-description-background-color`  | Tweakpane input foreground       |
| `stuiDescriptionForegroundColor` | `--stui-description-foreground-color`  | Tweakpane base background        |
| `stuiDescriptionFontSize`        | `--stui-description-font-size`         | Inherited Tweakpane font size    |
| `stuiDescriptionMaxWidth`        | `--stui-description-max-width`         | `min(16rem, calc(100vw - 16px))` |
| `stuiDescriptionPadding`         | `--stui-description-padding`           | `2px 4px`                        |
| `stuiDescriptionDelay`           | `--stui-description-delay`             | `500ms`                          |
| `stuiDescriptionFadeInDuration`  | `--stui-description-fade-in-duration`  | `0ms`                            |
| `stuiDescriptionFadeOutDuration` | `--stui-description-fade-out-duration` | `500ms`                          |

For more extensive styling, target the `.stui-description` class or an individual description's
generated `stui-description-*` ID.

Descriptions are plain text, and line breaks are preserved.

Described controls receive a `data-stui-description` attribute. If you want a visible affordance,
you can add one for an entire application without adding configuration to each component:

```css
:global([data-stui-description] > .tp-lblv_l)::after {
  content: ' 🛈';
}
```
