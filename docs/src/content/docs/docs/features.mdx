...
<script lang="ts">
  ...
  const theme: Theme = {
    ...
    stuiDescriptionHintDisplay: 'inline',
...
Set `stuiDescriptionHintDisplay` to `inline` to append ⓘ to described labels with a CSS `::after` pseudo-element. The hint follows the text and can disappear behind the ellipsis when the label is truncated. Its padding is part of the hover target. This is a CSS `display` value, so use `none` and `inline` rather than booleans.
