...
Modern [CSS color formats](https://www.w3.org/TR/css-color-4/) color formats, [named-color](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/named-color) strings, color arrays, and a wider range of color objects are supported via the experimental `<ColorPlus>` component. (This wraps my [ColorPlus Tweakpane plugin](https://github.com/kitschpatrol/tweakpane-plugin-color-plus).)
