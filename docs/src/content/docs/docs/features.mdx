...
| -------------------------------- | -------------------------------------- | -------------------------------- |
| `stuiDescriptionHint`            | `--stui-description-hint`              | `''` (CSS: `none`)               |
...
Set `stuiDescriptionHint` to plain text such as `'(i)'` or `'ⓘ'` to append a hint with the label's CSS `::after` pseudo-element. It defaults to `''`, which shows no hint at all.

When setting the CSS variable directly, use CSS content syntax: `--stui-description-hint: "(i)" / "";`, or `none` to hide it. The empty alternative text keeps the decorative hint out of accessible label text.
