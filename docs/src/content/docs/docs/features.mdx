...
The description appears in a compact, theme-aware hint after a short delay when text in the
control's label is hovered, positioned beside the pointer. When the control receives keyboard
focus, the hint is anchored to the control instead. For an unlabeled control, the whole blade is the
hover target. It uses the active theme's built-in slider value tooltip styling and the Popover API's
top layer, so it won't be clipped by the pane. A theme-matched caret points back to the originating
label or focused control:
...
```css
:global(.svelte-tweakpane-ui [data-stui-description] > .tp-lblv_l) {
  position: relative;
  padding-right: calc(var(--cnt-hp) + 1em);
  white-space: nowrap;
}

:global(.svelte-tweakpane-ui [data-stui-description] > .tp-lblv_l)::after {
  content: '🛈';
  position: absolute;
  right: var(--cnt-hp);
  width: 1em;
...
The reserved padding keeps the indicator visible after an ellipsis. Its generated box is included in
the description's hover target alongside the visible label text.
