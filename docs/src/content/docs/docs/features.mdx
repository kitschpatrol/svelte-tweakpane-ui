...
The theme key and CSS variable accept the same CSS `content` value. For example, `stuiDescriptionHint: '"(i)" / ""'` is equivalent to `--stui-description-hint: "(i)" / "";`. The empty alternative text keeps the decorative hint out of accessible label text.
