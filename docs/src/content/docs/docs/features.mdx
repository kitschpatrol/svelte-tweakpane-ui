...
import SvelteTweakpaneDescriptionExampleCode from '../../../examples/docs/SvelteTweakpaneDescriptionExample.md'
import SvelteTweakpaneDescriptionExample from '../../../examples/docs/SvelteTweakpaneDescriptionExample.svelte'
import SvelteTweakpaneDescriptionThemeExampleCode from '../../../examples/docs/SvelteTweakpaneDescriptionThemeExample.md'
import SvelteTweakpaneDescriptionThemeExample from '../../../examples/docs/SvelteTweakpaneDescriptionThemeExample.svelte'
...
_Experimental! Descriptions require a modern browser with the Popover API and CSS anchor positioning._

Requests for tooltips have come up a lot both in Svelte Tweakpane UI and the upstream project:

- https://github.com/kitschpatrol/svelte-tweakpane-ui/issues/28
- https://github.com/cocopon/tweakpane/issues/391
- https://github.com/cocopon/tweakpane/issues/559
- https://github.com/cocopon/tweakpane/pull/659

Tweakpane components accept an optional `description` prop which populates a tooltip-esque popover on your control's label.

The whole label is the hover target. If your control is unlabeled, then the whole blade becomes the hover target.

`<Pane>`, `<Folder>`, and `<TabPage>` components accept a `description` as well, with the title bar or the tab's button as the hover target. A pane needs a `title` for its description to show. Like other disabled controls, a disabled folder still shows its description.

These are defined to function much like native `title`-style hover tooltips, but they're styled to match your Tweakpane theme, and have pulled their design language from existing tooltip like pop-up elements in Tweakpane, such as those shown when scrubbing over graph views or slider values.

<ExampleWrapper>
  <SvelteTweakpaneDescriptionExample client:load />
</ExampleWrapper>

<details>
  <summary>See example code</summary>

  <SvelteTweakpaneDescriptionExampleCode />
</details>

The default hover delay is 500ms, but this and several other aspects of their behavior can be overridden via theme keys or their corresponding CSS variables:

<ExampleWrapper>
  <SvelteTweakpaneDescriptionThemeExample client:load />
</ExampleWrapper>

<details>
  <summary>See example code</summary>

  <SvelteTweakpaneDescriptionThemeExampleCode />
</details>

The corresponding inherited CSS custom properties can also be set directly on a pane or any of its ancestors:

| Theme key                        | CSS custom property                    | Default                          |
| -------------------------------- | -------------------------------------- | -------------------------------- |
| `stuiDescriptionHint`            | `--stui-description-hint`              | `none`                           |
| `stuiDescriptionMaxWidth`        | `--stui-description-max-width`         | `min(16rem, calc(100vw - 16px))` |
| `stuiDescriptionDelay`           | `--stui-description-delay`             | `500ms`                          |
| `stuiDescriptionFadeInDuration`  | `--stui-description-fade-in-duration`  | `50ms`                           |
| `stuiDescriptionFadeOutDuration` | `--stui-description-fade-out-duration` | `250ms`                          |

The theme key and CSS variable accept the same CSS `content` value. For example, `stuiDescriptionHint: '"ⓘ"'` is equivalent to `--stui-description-hint: "ⓘ";`. (The nested quotes in the theme value are required!)

The hint, if defined, is included at the end of the label text. It's hidden by default with the CSS value `none`.

Description values are treated as plain text, and explicit line breaks are preserved. Longer text wraps into balanced lines, and the tooltip fits the longest rendered line plus padding.

`stuiDescriptionMaxWidth` sets the upper limit used for wrapping. Tooltips also stay within the viewport, accounting for the pane's scale, and resize automatically when the description, styling, or available space changes.
...
Modern [CSS color formats](https://www.w3.org/TR/css-color-4/), [named-color](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/named-color) strings, color arrays, and a wider range of color objects are supported via the experimental `<ColorPlus>` component. (This wraps my [Color Plus Tweakpane plugin](https://github.com/kitschpatrol/tweakpane-plugin-color-plus).)
...
The [`hidden`](https://tweakpane.github.io/docs/api/classes/Pane.html#hidden) / [visibility](https://tweakpane.github.io/docs/misc/#visibility) accessor is not exposed, because visibility of a pane and its components may be managed directly in Svelte by adding / removing elements from the Svelte component markup through `{#if}` conditionals and similar.
...
_Svelte Tweakpane UI_ fully manages the lifecycle of the underlying Tweakpane objects, obviating the need for manual calls to [dispose](https://tweakpane.github.io/docs/misc/#disposing).
