...
If your control is unlabeled, then the whole blade becomes the hover target.
...
<script lang="ts">
  ...
  const theme: Theme = {
    ...
    stuiDescriptionHintDisplay: 'block',
...
| -------------------------------- | -------------------------------------- | -------------------------------- |
| `stuiDescriptionHintDisplay`     | `--stui-description-hint-display`      | `none`                           |
...
Set `stuiDescriptionHintDisplay` to `block` to show the built-in 🛈 after described labels. The hint participates in label layout only while displayed, so it follows short text directly and remains visible after an ellipsis when text is truncated. Its padded gap is part of the hover target. This is a CSS `display` value, so use `none` and `block` rather than booleans.

For more extensive styling intervention, target `.stui-description` or `.stui-description-hint`. Described controls receive a `data-stui-description` attribute.
