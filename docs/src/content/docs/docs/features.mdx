...
<Slider
  ...
  max={1}
  label="Exposure"
  description="Controls the overall scene brightness."
/>
```

The default hover delay is 500ms, but this and several other aspects of their behavior can be overridden with via theme keys or their corresponding CSS variables:

```svelte
<script lang="ts">
  import { Pane, type Theme, ThemeUtils } from 'svelte-tweakpane-ui'

  const theme: Theme = {
    ...ThemeUtils.presets.iceberg,
    stuiDescriptionHint: '"ⓘ"',
    stuiDescriptionMaxWidth: '20rem',
    stuiDescriptionDelay: '350ms',
...
The theme key and CSS variable accept the same CSS `content` value. For example, `stuiDescriptionHint: '"(i)"'` is equivalent to `--stui-description-hint: "(i)";`. The hint is included in accessible label text, and the description itself is connected to the control with `aria-describedby`.

Both default to `none`, which generates no hint or padding. To hide it explicitly, use `stuiDescriptionHint: 'none'` or `--stui-description-hint: none;`. A CSS empty string (`""`) still generates an empty pseudo-element with padding.

For more extensive styling intervention, target `.stui-description` or `[data-stui-description] > .tp-lblv_l::after`. Described controls receive a `data-stui-description` attribute.

Descriptions are currently treated as plain text, and line breaks are preserved.
