...
`<Folder>` and `<TabPage>` components accept a `description` as well, with the folder's title bar or the tab's button as the hover target. Like other disabled controls, a disabled folder still shows its description.
