...
The description appears in a compact, theme-aware hint when the control is hovered or receives
keyboard focus. It uses the Popover API's top layer and CSS anchor positioning, so it stays attached
to its control without being clipped by the pane. Browsers without the Popover API receive the same
text as a native `title` fallback:
